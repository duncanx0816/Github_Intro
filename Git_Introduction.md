...
- 生成SSH秘钥及相关配置 `ssh-keygen -t rsa -C 你的github认证邮箱`<br>
  在`.ssh`目录下面有三个文件，`id_rsa`，`id_rsa.pub`，`known_hosts`。

打开我们的github，进入设置页面： ![](http://i.imgur.com/XO29xC8.png) 至此，SSH秘钥就配置完成了，git上配置github环境也完成了。
...
在`rebase`加上`-i`会提供交互式的变基操作，在交互式操作中常用命令： ![](http://i.imgur.com/zD7PgjP.png)
